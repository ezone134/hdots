...
//!
//! Both fall back to `/tmp/tw_hypr_conf/{states,states2}` when the env vars
...
pub fn card_scene_path(card_id: &str) -> PathBuf {
    config_root().join("ui").join("cards").join(format!("{card_id}.ron"))
}
...
pub fn states_dir() -> PathBuf {
    PathBuf::from(std::env::var("states").unwrap_or_else(|_| "/tmp/tw_hypr_conf/states".into()))
}

/// `$states2` — volatile runtime state (never syncs to persist).
pub fn states2_dir() -> PathBuf {
    PathBuf::from(std::env::var("states2").unwrap_or_else(|_| "/tmp/tw_hypr_conf/states2".into()))
}
