...
impl Shell {
        ...
        // A declared `lock` surface in `shell.ron` owns the hero chrome — dim
        // backdrop, hero clock + date, avatar disc — while the password field
        // + auth error/caps + hold-power buttons + hint line stay in Rust via
        // the `lock_screen` Ink (`layout_lock_body`). The authored RON is
        // styled for the 1920×1080 dev target (center-anchored offsets), so
        // the surface is consulted only at that height; any other monitor (or
        // no file) runs the built-in Rust layout below unchanged.
        if (h - 1080.0).abs() < 24.0 {
            let vals = self.scene_values();
            if self.draw_shell_surface("lock", v, w, h, pal, &vals, None) {
                return;
            }
        }
        // dim backdrop
    ...
    /// Body painter for a declared `lock` surface: the password field + auth
    /// error / caps warning + the hold-to-confirm power buttons + the hint
    /// line — everything below the hero chrome. Also the tail half of the
    /// fallback `layout_lock`. Owns its own hit regions (the declared scene
    /// leaves these fully to Rust).
    pub(crate) fn layout_lock_body(&mut self, v: &mut Vec<Cmd>, w: f32, h: f32, pal: &Pal) {
        let ay = h * 0.20 + self.scale.s(144.0);
        // password field (centered — the field's own middle matches w/2)
