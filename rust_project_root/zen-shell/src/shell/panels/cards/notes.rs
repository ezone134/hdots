...
if !self.scene_owns_header && self.card_show_title { ui::title(v, x + self.scale.s(14.0), y + self.scale.s(10.0), "Notes", self.scale.fs(11.5), pal.fg); }
        ...
        // the scene owns the note list when notes.ron declares a `Rows` item
        // (also hidden while composing via its `visible` toggle, so the list
        // never draws over the composer); the empty hint + `+` stay Ink chrome
