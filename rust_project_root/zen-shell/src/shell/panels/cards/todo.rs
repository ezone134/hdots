...
impl Shell {
        ...
        let tw_ = w;
        let th_ = h;
if !self.scene_owns_header && self.card_show_title { ui::title(v, tx + self.scale.s(14.0), ty + self.scale.s(10.0), "To-Do", self.scale.fs(11.5), pal.fg); }
        ...
        // the scene owns the checklist rows when todo.ron declares a `Rows`
        // item (checkbox cells, hover ✕, row/del regions all scene-side) —
        // `drawn`/`start` below still feed the "+N more" footer
        ...
        // bottom composer strip: the scene owns it (a `Composer` item) once
        // todo.ron declares one — no double-draw, no duplicate regions
        if !self.scene_owns_composer {
