...
impl Shell {
    ...
    /// Whether the mirror card's pagination dots should show right now —
    /// shared by the Rust drawer and the declared-scene `Dots.when` binding
    /// (cursor rests on the card, edit mode off).
    pub(crate) fn mirror_dots_hover(&self) -> bool {
        let (x, y, w, h) = self.mirror_rect;
        !self.dash_edit
            && w > 0.0
            && self
                .cursor
...
if !self.scene_owns_header && self.card_show_title { ui::title(v, if self.card_show_glyph { x + self.scale.s(30.0) } else { x + self.scale.s(12.0) }, y + self.scale.s(9.0), "Mirror", self.scale.fs(12.0), pal.fg); }
        ...
        let hover = self.mirror_dots_hover();
