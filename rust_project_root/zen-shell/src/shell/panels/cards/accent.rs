...
impl Shell {
    ...
    pub(crate) fn draw_accent_card(&mut self, v: &mut Vec<Cmd>, x: f32, y: f32, w: f32, h: f32, pal: &Pal) {
        ...
        self.card_head(v, pal, x, y, w, self.scale.s(12.0), "Theme & Accent", Some(ICON_STICKY), None);
        ...
        if self.accent_list_open {
            ...
            ui::text(v, x + self.scale.s(14.0), cy + self.scale.s(6.5), ICON_DROP, self.scale.fs(9.5), if chov { pal.acc } else { pal.fg }, true);
            ui::text(v, x + self.scale.s(30.0), cy + self.scale.s(6.5), "Custom color", self.scale.fs(9.5), if chov { pal.acc } else { pal.fg }, false);
            self.region(x + self.scale.s(10.0), cy - self.scale.s(2.0), w - self.scale.s(20.0), self.scale.s(30.0), ck);
            // one-column paged list
            let ly = rows_top + self.scale.s(64.0);
