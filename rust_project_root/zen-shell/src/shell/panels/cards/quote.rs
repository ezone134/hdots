...
if !self.scene_owns_header && self.card_show_title { ui::title(v, if self.card_show_glyph { x + pad + self.scale.s(17.0) } else { x + pad }, y + self.scale.s(9.0), "Quote", self.scale.fs(12.0), pal.fg); }
        ...
        if self.quote_state == 1 {
            ...
            // the scene's `TextWrap` owns the wrapped body when declared —
            // nothing to draw here then (and the fallback below must not run)
