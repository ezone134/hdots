...
if !self.scene_owns_header && self.card_show_title { ui::title(v, if self.card_show_glyph { x + pad + self.scale.s(17.0) } else { x + pad }, y + self.scale.s(9.0), "Recent files", self.scale.fs(12.0), pal.fg); }
    ...
    /// Refresh `recent_files` when the 30 s cache is stale. Called by the Rust
    /// drawer and, since the card is fully declarative, by the scene path so
    /// the data stays live without any Rust body painter.
    pub(crate) fn refresh_recent_if_stale(&mut self) {
