...
/// "Custom color" picker row at the top of the Accent-card subview — runs the
/// gcp picker GUI (via `custom_color_main`).
pub(crate) const ACC_LIST_COLOR_KEY: u32 = 32_102;
...
/// Custom-Accent dashboard card view toggle (top-right icon): 32_500.
pub(crate) const CUSTOMACC_VIEW_KEY: u32 = 32_500;
/// Custom-Accent dashboard card search-bar focus: 32_501.
pub(crate) const CUSTOMACC_SEARCH_KEY: u32 = 32_501;
/// Base key for Custom-Accent dashboard card accents: 33_100 + absolute
/// (filtered) index. Kept below the banner band (34_000) and above Viz
/// (33_000) / Accent (32_000) so ranges never overlap.
pub(crate) const CUSTOMACC_KEY_BASE: u32 = 33_100;
/// top Custom-Accent accent key (fixed pattern bound; 900 slots).
pub(crate) const CUSTOMACC_KEY_MAX: u32 = 33_999;
/// Custom-Accent card layout (px before grid scale): search bar height.
pub(crate) const CUSTOMACC_SEARCH_H: f32 = 26.0;
/// Custom-Accent card layout: content area top from the card top edge.
pub(crate) const CUSTOMACC_CONTENT_Y: f32 = 64.0;
/// Custom-Accent card layout: list-view row height.
pub(crate) const CUSTOMACC_ROW_H: f32 = 26.0;
/// Custom-Accent card layout: grid-view circle diameter.
pub(crate) const CUSTOMACC_CELL: f32 = 30.0;
/// Custom-Accent card layout: grid-view horizontal gap between circles.
pub(crate) const CUSTOMACC_GAP_X: f32 = 10.0;
/// Custom-Accent card layout: grid-view vertical pitch (circle + label).
pub(crate) const CUSTOMACC_PITCH_Y: f32 = 46.0;
...
impl GridScale {
    ...
    /// Inverse of [`GridScale::s`] — unscale a panel px back to base px (the
    /// declarative responsive ladders reason in base px but read the scaled
    /// card box).
    pub fn base(self, px: f32) -> f32 { px / self.0 }
...
pub enum DashCard {
    ...
    Accent,
    /// Searchable custom-accent browser: live-filter search bar (top-middle),
    /// list/grid view toggle (top-right icon); clicking an accent activates it.
    CustomAcc,
...
DashCard::Notes => "notes",
    ...
    ///
    /// DECLARATIVE FIRST: if a scene file exists for this card
    /// (`~/.config/zen-shell/ui/cards/<id>.ron`) it is drawn instead and the
    /// Rust drawer is skipped (any `Ink` items still land in their Rust
    /// painters). The Rust function remains the fallback and the source of
    /// live values when no scene is authored yet.
    ///
    /// HYBRID cards manage their own scene chrome inside the Rust drawer
    /// (e.g. audiorec draws scene chrome + a live Rust fader/button) and are
    /// excluded from the whole-card interception so nothing dynamic falls out.
    pub(crate) fn draw(&self, shell: &mut Shell, v: &mut Vec<Cmd>, x: f32, y: f32, w: f32, h: f32, pal: &Pal, vals: &crate::scene::SceneValues) {
        if !matches!(self, DashCard::AudioRec) {
            let id = self.id();
            if shell.draw_card_scene(id, v, x, y, w, h, pal, vals) {
                return;
            }
        }
        self.draw_rust(shell, v, x, y, w, h, pal);
    }

    /// The built-in Rust painter for this card. Serves two roles: the
    /// fallback when no `.ron` scene exists, and the `Ink(name: "<id>")`
    /// body painter when a scene does exist (declarative chrome + Rust body,
    /// the QML Canvas analog).
    pub(crate) fn draw_rust(&self, shell: &mut Shell, v: &mut Vec<Cmd>, x: f32, y: f32, w: f32, h: f32, pal: &Pal) {
        match self {
            ...
            DashCard::CustomAcc => shell.draw_customacc_card(v, x, y, w, h, pal),
...
pub struct Shell {
    ...
    /// $states2/cur_scheme (synced by fetch_color_scheme / scheme_body)
    ...
    /// set by the ">" chevron on a theme card; app.rs runs `scheme_main fetch
    /// <name>` and stores the parsed detail into `theme_detail`
    pub pending_theme_fetch: Option<String>,
    /// fetched full color scheme shown when the ">" chevron was pressed
    /// (None = theme grid). Dual themes carry `light` too.
    pub theme_detail: Option<crate::themes::ThemeDetail>,
    ...
    /// (kind ∈ wall / default / custom). Writes the
    ...
    /// Custom-Accent dashboard card: live search query. None = search bar
    /// unfocused (no filtering); Some(q) = focused, accents filtered by the
    /// case-insensitive substring `q`.
    pub customacc_search: Option<String>,
    /// Custom-Accent dashboard card: true = list rows, false = grid of circles.
    pub customacc_list_view: bool,
    /// first visible (filtered) accent index in the Custom-Accent card
    pub customacc_scroll: usize,
    /// content viewport of the Custom-Accent card (x, y, w, h); wheel-driven
    pub customacc_rect: (f32, f32, f32, f32),
    ...
    pub acc_scrim: bool,
    /// Settings → Appearance: when true, accent-source clicks (wall / scheme
    /// default) pass the `all` flag so scheme_main writes every channel's
    /// `acc_source_*`; never applied to custom accent clicks. Showed + toggled
    /// only while `$states2/s != n` (i.e. a multi-state accent exists).
    pub acc_all_states: bool,
    /// a "custom color" pick (Settings or the theme card) queued for
    /// `custom_color_main` — a wrapper that runs the gcp picker GUI from
    /// `$sources`, stores the picked hex as `custom_acc_${s}` (source `c`),
    /// marks acc_changed and restores. app.rs spawns it detached.
    pub pending_custom_color: bool,
    ...
    /// the declared `dashboard` surface owns the viewing strip band (the Rust
    /// `layout_dash_grid` skips its own band to avoid double-painting; the
    /// edit-mode chrome mask is unrelated and always Rust). Set by the
    /// `dashboard` consult in `layout_expanded`, consumed by the grid painter.
    pub dash_band_via_scene: bool,
    /// the declared `dashboard` surface owns the viewing strip CHIPS (the
    /// Rust `layout_banner` skips its whole chip loop to avoid
    /// double-painting; edit-mode chrome — tray, guides, grips — stays Rust).
    /// Set per frame by `publish_banner_cells` (down when cells weren't
    /// published), consumed by the banner painter.
    pub dash_chips_via_scene: bool,
    ...
    /// declarative card-scene cache (`~/.config/zen-shell/ui/cards/*.ron`),
    /// reloaded on mtime change by the runtime watcher tick.
    pub(crate) scene_cache: crate::scene::SceneCache,
    /// whole-shell declarative scene cache (`~/.config/zen-shell/shell.ron`):
    /// named surfaces + component templates, reloaded on mtime change like
    /// `scene_cache`. Surfaces it declares replace their Rust draw paths.
    pub(crate) shell_scene: crate::scene::ShellSceneCache,
    /// click actions harvested from the current frame's scene draw
    /// (`key → action`), dispatched by `click()` for keys no Rust handler owns.
    pub(crate) scene_click_actions: std::collections::HashMap<u32, crate::scene::SceneAction>,
    /// set while drawing a card whose scene declares a `Header` — Rust card
    /// drawers skip their own title chrome so the scene owns it (no double-draw)
    pub(crate) scene_owns_header: bool,
    /// set while drawing a card whose scene declares a `Composer` — Rust card
    /// drawers skip their own bottom-input strip so the scene owns it
    pub(crate) scene_owns_composer: bool,
    /// set while drawing a card whose scene declares a `Rows` item — the
    /// To-Do card skips its own checklist row loop so the scene owns it
    pub(crate) scene_owns_rows: bool,
