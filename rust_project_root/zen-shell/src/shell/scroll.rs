...
impl Shell {
    ...
    /// Visible count of filtered accents in the Custom-Accent browser card
    /// (list view = rows that fit the viewport).
    pub(crate) fn customacc_list_visible(&self) -> usize {
        let (_, _, _, hh) = self.customacc_rect;
        (((hh - self.scale.s(2.0)) / self.scale.s(crate::shell::CUSTOMACC_ROW_H)).floor() as usize).max(1)
    }
    /// Grid stride of the Custom-Accent browser card: (cols, rows).
    pub(crate) fn customacc_grid_stride(&self) -> (usize, usize) {
        let (_, _, w, hh) = self.customacc_rect;
        let cols = (((w - self.scale.s(8.0)) / self.scale.s(crate::shell::CUSTOMACC_CELL + crate::shell::CUSTOMACC_GAP_X)).floor() as usize).max(1);
        let rows = (((hh + self.scale.s(2.0)) / self.scale.s(crate::shell::CUSTOMACC_PITCH_Y)).floor() as usize).max(1);
        (cols, rows)
    }
    /// Wheel scroll the Custom-Accent browser card. Grid mode scrolls whole
    /// rows (item-index step = cols).
    pub(crate) fn customacc_scroll_by(&mut self, notches: i32) -> bool {
        let names = self.customacc_filtered();
        if names.is_empty() {
            return false;
        }
        if self.customacc_list_view {
            let visible = self.customacc_list_visible();
            Self::list_scroll_by(&mut self.customacc_scroll, names.len(), visible, notches)
        } else {
            let (cols, rows) = self.customacc_grid_stride();
            let max = names.len().saturating_sub(cols * rows);
            let old = self.customacc_scroll;
            self.customacc_scroll = ((self.customacc_scroll as i32 + notches * cols as i32)).clamp(0, max as i32) as usize;
            old != self.customacc_scroll
        }
    }
    ...
    pub(crate) fn load_power_save(&mut self) {
        self.power_save_on = self.power_save_state();
    }

    /// The same read without the write, for the declarative battery cards'
    /// bindings (the scene owns the card, so the drawer's `load_power_save`
    /// never runs — `scene_values` is `&self`).
    pub(crate) fn power_save_state(&self) -> bool {
        let ch = crate::vars::read_channel();
        std::fs::read_to_string(crate::vars::states_dir().join(format!("power_save_{ch}")))
            .map(|s| s.trim() == "1")
            .unwrap_or(false)
    }
    ...
    /// True when the Wallpaper-card grid currently overflows its card and
    /// would actually scroll (uses the last-drawn card rect geometry). False
    /// for a card with nothing below the fold — most importantly so the wheel
    /// is NOT eaten when there's nothing to scroll.
    pub(crate) fn wp_card_scrollable(&self) -> bool {
        let (_, _, w, h) = self.wp_card_rect;
        if w <= 0.0 || h <= 0.0 {
            return false;
        }
        ...
        self.wp_card_max_scroll(cols, rows) > 0
    }
