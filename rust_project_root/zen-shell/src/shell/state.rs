...
impl Shell {
    ...
    // boot-time palette fallbacks (the live values come from
        ...
        let mut shell = Shell {
            ...
            customacc_search: None,
            customacc_list_view: true,
            customacc_scroll: 0,
            customacc_rect: (0.0, 0.0, 0.0, 0.0),
            ...
            acc_scrim: false,
            acc_all_states: false,
            pending_custom_color: false,
            start_icon_tone: 0,
            bg_alpha: 0xff,
            acc_alpha: 0xe5,
            scrim_alpha: 0x4c,
            ...
            sv_success: 0xff66bb6a,
            sv_warning: 0xffffa726,
            sv_danger: 0xffef5350,
            sv_info: 0xff42a5f5,
            sv_series: [
...
world_dl_pending: None,
    ...
    /// launcher appear instantly.  Called once, on the staged 0 ms boot-seed
    /// timer in `start()` once the event loop is running (never pre-loop).
    pub(crate) fn seed_data(&mut self) {
        ...
        self.refresh_pack();
        // live colors + dark/light state from /tmp/tw_rconf/states2
    ...
    pub(crate) fn set_mode(&mut self, mode: Mode) {
        ...
        // leaving the theme picker clears any open color-detail subview
        if mode != Mode::Themes {
            self.theme_detail = None;
            self.pending_theme_fetch = None;
        }
    ...
    pub(crate) fn reload_states_colors(&mut self) -> bool {
        ...
        set(&mut self.sv_acc_us, get("acc_us"));
        set(&mut self.sv_acc_hc, get("acc_hc"));
        set(&mut self.sv_acc_ec, get("acc_ec"));
        set(&mut self.sv_sfg, get("sfg"));
        set(&mut self.sv_sfg_ec, get("sfg_ec"));
        set(&mut self.sv_sfg_hc, get("sfg_hc"));
        set(&mut self.sv_sfg_us, get("sfg_us"));
        set(&mut self.sv_border, get("border"));
        set(&mut self.sv_hover, get("hover"));
        set(&mut self.sv_focus, get("focus"));
        set(&mut self.sv_disabled, get("disabled"));
        set(&mut self.sv_success, get("success"));
        set(&mut self.sv_warning, get("warning"));
        set(&mut self.sv_danger, get("danger"));
        set(&mut self.sv_info, get("info"));
...
impl Shell {
    ...
    /// Indices into `custom_accs` matching the Custom-Accent card's live
    /// search query (case-insensitive substring on the name; empty query =
    /// all). Stable ordering — the card keys by absolute filtered index.
    pub(crate) fn customacc_filtered(&self) -> Vec<usize> {
        let q = self.customacc_search.as_deref().unwrap_or("").to_lowercase();
        (0..self.custom_accs.len())
            .filter(|&i| q.is_empty() || self.custom_accs[i].name.to_lowercase().contains(&q))
            .collect()
    }
