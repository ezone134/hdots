    ...
    pub(crate) fn click(&mut self, x: f32, y: f32) -> Option<Mode> {
        ...
        match self.mode {
            ...
            Mode::Expanded => {
                ...
                k @ ACC_LIST_KEY_BASE..=ACC_LIST_KEY_MAX => {
                    ...
                    if let Some(a) = self.custom_accs.get(top + j) {
                        self.pending_accent_src = Some(("custom_acc".into(), a.name.clone()));
                    }
                    self.accent_list_open = false;
                    None
                }
            ...
            Mode::Settings => match key {
                ...
                // scheme-default. Handled by app.rs via `theme_main accent`.
                60 => {
                    self.pending_accent_src = Some(("acc_from_wall".into(), String::new()));
                    None
                }
                61 => {
                    self.pending_accent_src = Some(("theme_default".into(), String::new()));
                    None
                }
                // Custom accent → acc_source = c
                223 => {
                    self.pending_accent_src = Some(("custom_acc".into(), String::new()));
                    None
                }
                ...
                // Dropdown item: select + apply. `theme_main accent custom_acc
