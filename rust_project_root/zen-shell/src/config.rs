...
# timeout_ms = 10000
# Declarative per-card appearance spec — edit + `zen-shell ipc call config
# reload` (or just save; the shell watches the file) to restyle WITHOUT a
# rebuild. Color values are semantic tokens resolved against the live theme
# at draw time: fg, fg2, fg3, acc, sfg, raised, raised_hl, hover, hover_hl,
# acc_tint, hairline, ok, warn, danger, info. No hex here — theme switches
# keep recoloring spec cards too.
[cards.audiorec]
# record button surface idle / while recording
rec_surface = "raised"
rec_active_surface = "danger"
rec_dot = "danger"
...
# ── Declarative WHOLE-DASHBOARD layout spec ─────────────────────────────
# Every visual item of the dashboard is declared here, QML-style: the banner
# strip (chips + separators + zones), the parked banner-chip tray, the cards
# on the grid, and the parked-card tray. Edit + save → the shell re-reads the
# file (auto-reload) and rebuilds the layout. Missing sections keep the
# remembered per-channel layout; declaring `run` makes it authoritative.
[dashboard]
# geometry overrides ([bar] counterparts)
# padding = 12
# radius = 16
# card_radius = 16
# card_radius_sync = true

[dashboard.banner]
# banner strip tokens, left → right: chip names, `sep:<glyph>`, `zone:0|1|2`,
# `filler`. Declaring `run` overrides the strip layout entirely.
# run = ["workspaces", "sep:|", "title", "date", "clock", "filler", "wifi", "battery", "volume", "settings", "power"]
# parked = ["weather", "tray", "dnd", "cpu", "ram", "net_speed"]

[dashboard.cards]
# dashboard grid cards in packing order + cards parked in the add-tray.
# Declaring `run` makes the grid layout spec-authoritative.
# run = ["clock", "system", "weather", "media", "network", "audiorec"]
# parked = ["docker", "sshvpn", "currency", "worldmap"]
"##;
...
pub struct Config {
    ...
    /// declarative look spec for the audio recorder card — structure +
    /// semantic color tokens resolved against the live theme at draw time
    /// (no hex, no rebuilds to restyle).
    #[serde(default)]
    pub cards: Cards,
    /// Declarative whole-dashboard layout spec ([dashboard] section).
    /// Every visual item is declared here; the spec drives banner strip
    /// order, card layout, parked pools, and geometry at load time.
    #[serde(default)]
...
/// Declarative per-card appearance spec — the "spec file" the shell reloads
/// at runtime (`zen-shell ipc call config reload`, or automatically when the
/// per-state config mtime changes). Structure + semantic color tokens only;
/// logic stays in Rust. Colors are `ColorToken` names ("acc", "danger",
/// "raised", …) resolved against the live theme on every draw.
#[derive(Deserialize, Serialize, Clone, Default)]
#[serde(default)]
pub struct Cards {
    pub audiorec: CardAudioRec,
}

/// Audio recorder card spec (pane layout + semantic colors).
#[derive(Deserialize, Serialize, Clone)]
#[serde(default)]
pub struct CardAudioRec {
    /// record/stop button hover+idle surface — a `ColorToken` name
    pub rec_surface: crate::ui::ColorToken,
    /// record/stop button surface while actively recording
    pub rec_active_surface: crate::ui::ColorToken,
    /// record dot / "REC" glyph color
    pub rec_dot: crate::ui::ColorToken,
    /// recordings-list name ink
    pub row_ink: crate::ui::ColorToken,
    /// recordings-list row surface under a single row's hover
    pub row_hover: crate::ui::ColorToken,
    /// play glyph color on hover
    pub play_hover: crate::ui::ColorToken,
    /// delete glyph color on hover
    pub del_hover: crate::ui::ColorToken,
    /// armed-delete row surface (first confirm click arms it)
    pub del_armed: crate::ui::ColorToken,
    /// pane-0 record button vertical position inset from the card bottom (px)
    pub rec_pad_bottom: f32,
    /// recordings list row height (px)
    pub row_h: f32,
}

impl Default for CardAudioRec {
    fn default() -> Self {
        use crate::ui::ColorToken as T;
        CardAudioRec {
            rec_surface: T::Raised,
            rec_active_surface: T::Danger,
            rec_dot: T::Danger,
            row_ink: T::Fg2,
            row_hover: T::RaisedHl,
            play_hover: T::Acc,
            del_hover: T::Danger,
            del_armed: T::Danger,
            rec_pad_bottom: 12.0,
...
impl Cards {
    /// The live per-card spec for the audio recorder card.
    pub fn audiorec(&self) -> &CardAudioRec {
        &self.audiorec
    }
}

// ── Declarative whole-dashboard layout spec ([dashboard]) ──────────────
// Every visual item is declared here like a QML scene: banner strip tokens,
// parked banner chips, enabled dashboard cards, parked cards, and geometry.
// Colors use semantic tokens; positions are derived by the flow packer.

/// Declarative whole-dashboard layout spec ([dashboard] section).
#[derive(Deserialize, Serialize, Clone, Default)]
#[serde(default)]
pub struct DashboardSpec {
    pub banner: DashboardBannerSpec,
    pub cards: DashboardCardsSpec,
    /// extra space between window edges and dashboard cards (px).
    pub padding: Option<f32>,
    /// window corner roundness (px).
    pub radius: Option<f32>,
    /// card corner roundness (px).
    pub card_radius: Option<f32>,
    /// when true cards follow window roundness instead of own value.
    pub card_radius_sync: Option<bool>,
}

/// Banner strip spec: run = tokens on the strip (left→right),
/// parked = chips available in the click-to-add tray.
#[derive(Deserialize, Serialize, Clone, Default)]
#[serde(default)]
pub struct DashboardBannerSpec {
    /// strip tokens — chips, `sep:<glyph>`, `zone:0|1|2`, `filler`.
    pub run: Vec<String>,
    /// chips offered in the parked tray (click to add to strip).
    pub parked: Vec<String>,
}

/// Dashboard cards spec: run = cards on the grid (packed order),
/// parked = cards in the tray (click to add to grid).
#[derive(Deserialize, Serialize, Clone, Default)]
#[serde(default)]
pub struct DashboardCardsSpec {
    /// card ids shown on the dashboard, in packing order.
    pub run: Vec<String>,
    /// card ids in the parked tray.
    pub parked: Vec<String>,
}

impl DashboardSpec {
    /// True when the spec has any declarative content (non-empty run lists
    /// or explicit geometry).
    pub fn is_active(&self) -> bool {
        !self.banner.run.is_empty()
            || !self.banner.parked.is_empty()
            || !self.cards.run.is_empty()
            || !self.cards.parked.is_empty()
            || self.padding.is_some()
            || self.radius.is_some()
            || self.card_radius.is_some()
            || self.card_radius_sync.is_some()
    }
}
...
impl Config {
    pub fn load(path: &Path) -> Self {
        ...
        cfg.apply_dashboard_spec();
        cfg
    }

    /// Apply the `[dashboard]` spec onto the config in-place: when the spec
    /// declares geometry or a banner strip it becomes the source of truth,
    /// so the existing parsers (`banner_order()`, `ui_pad()`, …) pick it up
    /// without further changes. Cards are reconciled at state level (the
    /// flow packer derives spans there); nothing here needs DashCard.
    pub fn apply_dashboard_spec(&mut self) {
        let spec = self.dashboard.clone();
        if !spec.is_active() {
            return;
        }
        // geometry overrides
        if let Some(p) = spec.padding {
            self.bar.ui_pad = Some(p);
        }
        if let Some(r) = spec.radius {
            self.bar.win_radius = Some(r);
        }
        if let Some(r) = spec.card_radius {
            self.bar.card_radius = Some(r);
        }
        if let Some(s) = spec.card_radius_sync {
            self.bar.card_radius_sync = Some(s);
        }
        // banner strip: spec run wins over bar.banner_order
        if !spec.banner.run.is_empty() {
            self.bar.banner_order = Some(spec.banner.run);
        }
    }

    /// Declarative per-card appearance spec — see [`config::Cards`].
    pub fn audiorec(&self) -> &crate::config::CardAudioRec {
        self.cards.audiorec()
    }
...
mod tests {
    ...
    /// The [dashboard] spec parses and round-trips its banner/cards structure.
    #[test]
    fn dashboard_spec_roundtrip() {
        let toml = r#"
            [dashboard]
            padding = 14
            radius = 18
            card_radius = 12
            card_radius_sync = false

            [dashboard.banner]
            run = ["workspaces", "sep:|", "clock", "filler", "battery"]
            parked = ["weather", "dnd", "net_speed"]

            [dashboard.cards]
            run = ["clock", "system", "audiorec"]
            parked = ["docker", "currency"]
        "#;
        let cfg: Config = toml::from_str(toml).expect("dashboard spec parses");
        assert_eq!(cfg.dashboard.banner.run, vec!["workspaces", "sep:|", "clock", "filler", "battery"]);
        assert_eq!(cfg.dashboard.banner.parked, vec!["weather", "dnd", "net_speed"]);
        assert_eq!(cfg.dashboard.cards.run, vec!["clock", "system", "audiorec"]);
        assert_eq!(cfg.dashboard.cards.parked, vec!["docker", "currency"]);
        assert_eq!(cfg.dashboard.padding, Some(14.0));
        assert_eq!(cfg.dashboard.radius, Some(18.0));
        assert_eq!(cfg.dashboard.card_radius, Some(12.0));
        assert_eq!(cfg.dashboard.card_radius_sync, Some(false));
        assert!(cfg.dashboard.is_active());

        let s = toml::to_string_pretty(&cfg).expect("serialize dashboard spec");
        let back: Config = toml::from_str(&s).expect("round-trip");
        assert_eq!(back.dashboard.banner.run, cfg.dashboard.banner.run);
        assert_eq!(back.dashboard.cards.parked, cfg.dashboard.cards.parked);
    }

    /// apply_dashboard_spec pushes declared geometry + banner order into the
    /// bar so existing parsers (`ui_pad()`, `win_radius()`, `banner_order()`)
    /// pick them up; cards are left for state-level reconciliation.
    #[test]
    fn dashboard_spec_applies_to_bar() {
        let toml = r#"
            [dashboard]
            padding = 20
            radius = 22
            ...
            [dashboard.banner]
            run = ["workspaces", "title", "clock"]
        "#;
        let mut cfg: Config = toml::from_str(toml).expect("parses");
        cfg.apply_dashboard_spec();
        assert_eq!(cfg.ui_pad(), 20.0, "spec padding overrides bar pad");
        assert_eq!(cfg.win_radius(), 20.0, "spec radius overrides window roundness (≤20 clamp)");
        let tokens = cfg.banner_order();
        assert_eq!(tokens.len(), 3, "spec run replaces the default strip");
        assert!(matches!(&tokens[1], crate::shell::BannerToken::Chip(crate::shell::BannerItem::Title)));
        assert!(cfg.dash_cards.is_empty(), "cards reconciliation stays at state level");
    }

    /// An empty [dashboard] spec (no run lists) must not clobber the existing
    /// bar layout — legacy configs keep their remembered order.
    #[test]
    fn empty_dashboard_spec_is_inert() {
        let toml = r#"
            [dashboard]
            ...
            [dashboard.cards]
        "#;
        let mut cfg: Config = toml::from_str(toml).expect("empty spec parses");
        cfg.apply_dashboard_spec();
        assert!(!cfg.dashboard.is_active(), "no declared items → inactive");
        // banner falls back to the DEFAULT_BANNER_ORDER (has zone markers)
        let tokens = cfg.banner_order();
        assert!(tokens.iter().any(|t| matches!(t, crate::shell::BannerToken::Zone(_))));
        assert_eq!(cfg.ui_pad(), 12.0, "default pad untouched");
    }
}
