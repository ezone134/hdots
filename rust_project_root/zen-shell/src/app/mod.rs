...
impl App {
    ...
    pub fn run(cfg: Config) -> Result<(), String> {
        // Module values ($sources/lib/*.so) are loaded on their own thread and
        // published into a snapshot; start it before the first frame so the
        // opening layout already has them.
        crate::mods::start();
    ...
    fn new(
        ...
        // cliphist dbs / persisted history are seeded lazy in the deferred
        // boot-seed timers (~450 ms into the loop, see the staged timers in
        // `start()`) so rasterizing image thumbnails never delays the bar.
        ...
        // brightness, volume are seeded LAZILY in the deferred boot-seed
        // timers in `start()` (0 / 80 / 450 ms once the loop runs) — not here.
    ...
    pub fn start(&mut self, mut event_queue: wayland_client::EventQueue<App>) -> Result<(), String> {
        ...
        // First frame is up (bar + any boot-mode surface visible). HARD RULE
        // (see NEXT_STEPS.md): nothing that can block runs before the event
        // loop starts — the loop below is the ONLY thing that dispatches
        // hover/click, so any slow seed on this path is felt as "wait N
        // seconds until the pill becomes expandable" (cold D-Bus for
        // MPRIS/NM/BT, a wpctl/pacman spawn racing login, cliphist db reads).
        // All boot seeds moved to staged in-loop timers further down
        // (`deferred boot seeds`): the pill is interactive from the loop's
        // first dispatch and the data backfills within ~0.5 s.
        ...
        // DEFFERED BOOT SEEDS (hard rule, NEXT_STEPS.md): the pill must be
        // hover/click-expandable from the event loop's first dispatch. The
        // shell-level + peripheral seeds that used to run synchronously here
        // (before the loop) now run on staged in-loop one-shot timers, so a
        // slow backend can never hold the first frame's input hostage:
        //   0 ms    shell-file seeds (theme/flag/todo/hostname files) + pill re-fit
        //   80 ms   fast reads  (hyprctl ws/win, battery sysfs/upower, volume files)
        //   450 ms  heavy seeds (D-Bus services poll, cliphist bbolt db reads)
        for (i, ms) in [0u64, 80, 450].iter().enumerate() {
            let boot_tok = handle
                .insert_source(
                    calloop::timer::Timer::from_duration(Duration::from_millis(*ms)),
                    move |_, _, app: &mut App| {
                        let changed = match i {
                            0 => {
                                if std::env::var("ZEN_TRACE").is_ok() {
                                    eprintln!("zen: boot seeds (shell)");
                                }
                                // dashboard layout, colors, accent flags, todos,
                                // hostname, etc. (per-state config files)
                                app.shell.seed_data();
                                // Boot re-fit: the resting pill is sized to its
                                // contents; the seeds above filled in battery /
                                // AC / colors — the measured width may exceed
                                // the boot surface. Grow to it now (tiny morph)
                                // instead of leaving the pill clipped until the
                                // first expand⇄collapse.
                                if app.shell.mode == crate::shell::Mode::Collapsed
                                    || app.shell.mode == crate::shell::Mode::Expanded
                                {
                                    app.shell.refresh_size();
                                    app.ensure_morph_timer();
                                }
                                true
                            }
                            1 => app.seed_boot_data_quick(),
                            _ => app.seed_boot_data_heavy(),
                        };
                        if changed {
                            app.dirty = true;
                            app.maybe_render();
                ...
                .map_err(|e| format!("boot seed timer: {e:?}"))?;
            let _ = boot_tok;
        }
...
let mut changed = false;
    ...
    /// Quick peripheral seeds: fast local reads only (hyprctl IPC, sysfs/upower
    /// battery, brightness, volume files). Runs ~80 ms into the loop via the
    /// staged boot-seed timers in `start()` — never before the loop runs, never
    /// on the boot critical path. Returns true if a seed changed shell state.
    fn seed_boot_data_quick(&mut self) -> bool {
        let mut changed = false;
        if std::env::var("ZEN_TRACE").is_ok() {
            eprintln!("zen: boot seeds (quick)");
        }
    ...
    /// Heavy peripheral seeds: D-Bus services poll (MPRIS / NetworkManager /
    /// Bluetooth) + clipboard cliphist bbolt db reads with PNG thumbnail
    /// rasterization — the pieces that can block for seconds while the session
    /// is still coming up at login (a cold system bus, or wpctl racing
    /// PipeWire). Run ~450 ms into the loop via the boot-seed timers, long
    /// after the pill is already interactive. Returns true if anything loaded.
    fn seed_boot_data_heavy(&mut self) -> bool {
        let mut changed = false;
        if std::env::var("ZEN_TRACE").is_ok() {
            eprintln!("zen: boot seeds (heavy)");
        }
        ...
        // thumbnails) — only visible in the clipboard panels, so seeding it
        // late is invisible to the bar itself.
    ...
    fn run_pending_actions(&mut self) {
        ...
        // theme-card ">" chevron → `scheme_main fetch <name>`: pure stdout
        // JSON of the whole color scheme (no writes). Parse it into the shell
        // so the Themes panel shows the detail view on next render.
        if let Some(name) = self.shell.pending_theme_fetch.take() {
            if let Some(detail) = crate::themes::fetch_theme_detail(&name) {
                self.shell.theme_detail = Some(detail);
            }
            self.dirty = true;
        }
        // Settings → Appearance theme-accent click → `scheme_main accent <kind>
        // [payload]`. scheme_main_body writes `$states/acc_source_<ch>` (kind ∈
        // wall / default / custom; payload is "all" for wall/default only) and,
        // on an actual change, echoes `$states2/acc_changed` + restores so
        ...
        // Custom-color pick (Settings → Appearance → "Custom color", or the
        // theme card): spawns the `custom_color_main` wrapper, which runs the
        // gcp picker GUI from `$sources`, writes the picked hex as
        // `$states/custom_acc_${s}` + source `c`, marks acc_changed and
        // restores — same effect as pick_accent_main but via the gcp GUI.
        if std::mem::take(&mut self.shell.pending_custom_color) {
            use std::process::{Command, Stdio};
            let _ = Command::new("custom_color_main")
                .stdin(Stdio::null())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .spawn();
        ...
        // Paint the result NOW. This handler runs on pointer release, and the
        // release path does not call maybe_render for overlay modes (only hover
        // reconcile, which early-returns for Mode::Themes). Without this the
        // frame waits for an unrelated later event (next motion/timer tick) —
        // the theme-detail ">" felt slow to appear even though scheme_main
        // fetch itself is ~7 ms.
        self.maybe_render();
    }
...
mod tests {
    ...
    fn boot_seed_timing() {
        ...
        use crate::backends::{Power, SysfsPower, UPowerPower, SysfsBrightness, Brightness, WpctlAudio, Audio};
        let cfg = crate::config::Config::default();
        let t = Instant::now();
        let mut shell = crate::shell::Shell::new(cfg.clone());
        println!("Shell::new: {:?}", t.elapsed());

        let mut t = Instant::now();
        let hypr = crate::hypr::Hypr::new();
        println!("Hypr::new: {:?}", t.elapsed());
        t = Instant::now();
        let _n = hypr.update_shell(&mut shell);
        println!("hypr.update_shell: {:?} ({} changed)", t.elapsed(), _n);

        t = Instant::now();
        let mut power: Box<dyn Power> = Box::new(UPowerPower::new());
        println!("UPowerPower::new: {:?}", t.elapsed());
        t = Instant::now();
        let _c = power.poll(&mut shell);
        println!("UPowerPower::poll: {:?} (changed {_c})", t.elapsed());

        t = Instant::now();
        let mut s2 = SysfsPower::new();
        let _c = s2.poll(&mut shell);
        println!("SysfsPower::new+poll: {:?} (changed {_c})", t.elapsed());

        t = Instant::now();
        let mut bri = SysfsBrightness::new();
        let _g = bri.get();
        println!("SysfsBrightness::new+get: {:?}", t.elapsed());

        t = Instant::now();
        let mut au = WpctlAudio;
        let _m = au.default_volume();
        println!("WpctlAudio::default_volume: {:?}", t.elapsed());

        t = Instant::now();
        let mut svc = crate::services::Services::new();
        println!("Services::new: {:?}", t.elapsed());
        t = Instant::now();
        let _c = svc.poll(&mut shell);
        println!("Services::poll: {:?} (changed {_c})", t.elapsed());

        t = Instant::now();
        let mut clip = crate::clipboard::Clipboard::new();
        let mut img = crate::img::ImageStore::new();
        let _c = clip.seed_history(&mut shell, &mut img);
        println!("clipboard.seed_history: {:?} (changed {_c})", t.elapsed());

        t = Instant::now();
        shell.seed_data();
        println!("shell.seed_data: {:?}", t.elapsed());
    }

    #[test]
    fn dashboard_first_layout_timing() {
        use std::time::Instant;
        let cfg = crate::config::Config::default();
        let mut shell = crate::shell::Shell::new(cfg.clone());
        shell.seed_data();
        // expanded dashboard at a decent canvas size
        shell.set_mode(crate::shell::Mode::Expanded);
        let (tw, th) = shell.target_size();
        let _ = (tw, th);
        let (cw, ch) = (1024.0f32, 800.0f32);
        let t = Instant::now();
        let pal = crate::ui::Pal { fg: 0xe8e6e3ff, bg: 0x141414ff, acc: 0x4fc2ffff, sfg: 0x101010ff };
        let _v: Vec<crate::shell::Cmd> = shell.layout(cw, ch);
        println!("layout #1 (cold first expand): {:?}", t.elapsed());
        let t = Instant::now();
        let _v2: Vec<crate::shell::Cmd> = shell.layout(cw, ch);
        println!("layout #2: {:?}", t.elapsed());
        // a full expanded services tick: stats + card polls (first unthrottled run)
        let t = Instant::now();
        let mut stats = crate::stats::Stats::new();
        stats.poll(&mut shell);
        crate::stats::Stats::poll_top_procs(&mut shell);
        crate::stats::Stats::poll_active_win(&mut shell);
        crate::stats::Stats::poll_pkg_updates(&mut shell);
        crate::stats::Stats::poll_ssh_vpn(&mut shell);
        crate::stats::Stats::poll_docker(&mut shell);
        crate::stats::Stats::poll_kb_layout(&mut shell);
        let _ = shell.poll_smart();
        let _ = shell.poll_systemd();
        let _ = shell.poll_journal();
        let _ = shell.poll_conninfo();
        println!("expanded first tick (stats+card polls): {:?}", t.elapsed());
    }
