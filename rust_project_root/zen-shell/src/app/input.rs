                ...
                PointerEventKind::Axis { vertical, horizontal, .. } => {
                    ...
                    if steps != 0 {
                        ...
                        if self.shell.mode == crate::shell::Mode::Expanded {
                            ...
                            // custom-accent browser card (list or grid view)
                            if inside(self.shell.customacc_rect) && !self.shell.customacc_filtered().is_empty() {
                                card_consumed |= self.shell.customacc_scroll_by(steps);
                            }
                            ...
                            } else if (!inside(self.shell.wp_card_rect) || !self.shell.wp_card_scrollable())
