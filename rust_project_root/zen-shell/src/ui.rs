...
// ------------------------------------------------------------ spec tokens
//
// Semantic surface/ink names the declarative per-card spec (`[cards.*]` in
// the shell TOML) can mention instead of raw colors. Everything resolves
// against the LIVE theme `Pal` at draw time, so theme switches keep applying
// shell-wide even to spec-driven cards. Unknown tokens fall back to `fg`.

/// A compact, `Copy`-able dynamic color name for [`ColorToken::Value`] — up
/// to 24 bytes stored inline so the frequently-copied `ColorToken` stays
/// `Copy`. Serde reads/writes it as a plain string, so RON authors write
/// `value("dnd_chip")` exactly like any other token.
#[derive(Clone, Copy, PartialEq, Eq, Debug, Hash)]
pub struct DynColor(pub [u8; 24]);

impl DynColor {
    const MAX: usize = 24;

    /// Constructor for `'static` names (Rust-authored scene values) — panics
    /// on an over-long name so a typo surfaces in the first test run.
    pub fn named(name: &'static str) -> Self {
        Self::new(name).unwrap_or_else(|| panic!("DynColor name too long: {name}"))
    }
    ...
    pub fn as_str(&self) -> &str {
        let end = self.0.iter().position(|&b| b == 0).unwrap_or(Self::MAX);
        std::str::from_utf8(&self.0[..end]).unwrap_or("")
    }
}

impl std::fmt::Display for DynColor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}

impl serde::Serialize for DynColor {
    fn serialize<S: serde::Serializer>(&self, s: S) -> Result<S::Ok, S::Error> {
        s.serialize_str(self.as_str())
    }
}

impl<'de> serde::Deserialize<'de> for DynColor {
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        let name = String::deserialize(d)?;
        DynColor::new(&name)
            .ok_or_else(|| serde::de::Error::custom(format!("color name `{name}` must be 1..=24 chars")))
    }
}

/// A named color from the declarative card spec — resolved per frame against
/// the current theme `Pal`. Keeps hex out of the spec entirely.
#[derive(Clone, Copy, PartialEq, Debug, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ColorToken {
    /// primary ink
    Fg,
    /// secondary ink (60% fg→bg)
    Fg2,
    /// muted ink (82% fg→bg)
    Fg3,
    /// accent ink
    ...
    Sfg,
    /// raised (quiet) surface — same as `raised()`
    Raised,
    /// raised surface under hover
    RaisedHl,
    /// hover surface
    Hover,
    /// strong hover surface
    HoverHl,
    /// accent-tinted surface (active/selected wells)
    AccTint,
    /// faint selected-row wash (10% accent — the fps/mirror active row)
    SelBg,
    /// hairline surface edge
    Hairline,
    /// muted OK status green
    Ok,
    /// muted warning amber
    Warn,
    /// muted danger red (destructive actions)
    Danger,
    /// muted info blue
    Info,
    /// a state-driven dynamic color, bound per frame by name from the live
    /// shell values (`SceneValues` colors map). Lets a single element (DND
    /// chip accent, banner token) follow runtime state instead of a fixed
    /// token. Scene drawing resolves it via
    /// `scene::ColorTokenExt::resolve_with`; plain `resolve` degrades to `fg`
    /// so non-scene consumers (TOML spec tokens) stay safe.
    Value(DynColor),
}
...
impl ColorToken {
    /// Resolve the token against the live theme — one number, no allocations.
    pub fn resolve(self, pal: &Pal) -> u32 {
        match self {
            ColorToken::Fg => pal.fg,
            ColorToken::Fg2 => fg2(pal),
            ColorToken::Fg3 => fg3(pal),
            ColorToken::Acc => pal.acc,
            ColorToken::Sfg => pal.sfg,
            ColorToken::Raised => raised(pal),
            ColorToken::RaisedHl => raised_hl(pal),
            ColorToken::Hover => hover(pal),
            ColorToken::HoverHl => hover_hl(pal),
            ColorToken::AccTint => acc_tint(pal),
            ColorToken::SelBg => sel_bg(pal),
            ColorToken::Hairline => hairline(pal),
            ColorToken::Ok => OK,
            ColorToken::Warn => WARN,
            ColorToken::Danger => DANGER,
            ColorToken::Info => INFO,
            // dynamic tokens need the live values (`resolve_with`) to resolve;
            // theme-only consumers see the quiet default
            ColorToken::Value(_) => pal.fg,
        }
    }

    /// Hover variant of a surface token — a surface raised one step so an
    /// element reads as pressed/hovered. Surface tokens stay on theme; ink
    /// tokens resolve as-is (they're already hover-targeted by the spec).
    pub fn hover_variant(self, pal: &Pal) -> u32 {
        match self {
            ColorToken::Raised => raised_hl(pal),
            ColorToken::Hover => hover_hl(pal),
            ColorToken::AccTint => mix(pal.bg, pal.acc, 0.24),
            ColorToken::Hairline => hairline_hl(pal),
            // a dynamic color already carries its own hover intent; degrade
            // to fg under the plain (theme-only) resolver
            ColorToken::Value(_) => pal.fg,
