...
> **This file is a session log.** The header below (2026-09-15) and the sections
> under it record that one session and the phases after it. For *current* state
> read `NEXT_STEPS.md` (open items, build recipe, remaining Batch 3) and
> `CHANGELOG.md` (what landed, newest first).

## CURRENT STATE (2026-09-25) — supersedes the 2026-09-15 log below

- **304/304 tests green** (the suite read 258 before this session; ten tests had
  never been running).
- **Batch 3 has five cards in** (sliders, toggles, compositor, power, battery):
  - **`sliders` is a fully declarative zero-`Ink` card** (`ui/cards/sliders.ron`)
    — see CHANGELOG 2026-09-25 for the shape and the new bindings, and
    `sliders_rows_land_on_the_drawer_geometry` for the parity guard that pins
    the groove spans, row pitch, label anchors, reset-chip regions and both
    panes against the Rust drawer's numbers.
  - **`toggles` is a fully declarative zero-`Ink` card**
    (`ui/cards/toggles.ron`) — the six switch tiles are one new **`Tiles`**
    frame. The item owns the responsive column count (`cols_break`, the
    drawer's 225/330 px width ladder), the caption floor (`label_min_h` — a
    squat tile goes icon-only and centers the glyph), the on/hover/off ink
    ladder, and the Wi-Fi / BT ⋯ chip over a tall right-edge strip; the per-tile
    key / glyph / caption / on-state / chip-key ride a `SceneValue::Tiles` list
    published per frame, exactly like `Rows` / `Grid`.
  - **`compositor` is a fully declarative zero-`Ink` card**
    (`ui/cards/compositor.ron`) — header, three effect tiles in an equal-column
    `Row` of `Stack`s, and the two screenshot buttons whose row declares
    `min_card_h: 110` so a short card drops the row instead of overflowing. The
    tiles' four state inks (fill / dot / label / `on`-`off` caption) are
    per-frame values: a lit tile fills with the accent at 16% alpha and lifts to
    25% on hover, a blend no static token can spell.
  - **`powerh` / `powerv` are fully declarative zero-`Ink` cards**
    (`ui/cards/powerh.ron`, `ui/cards/powerv.ron`) — the five power actions as
    `Stack`s of a declared background + hairline, the hold-to-confirm liquid
    and the glyph, at the resident keys 12000..12004. The three generic engine
    features that made it possible (`Row.halign`, `Bar.vertical`,
    `Surface.stroke`) are reusable; the per-frame `power_*` bindings keep the
    drawer's full state ladder (background tint, danger glyph ink, hold
    fraction, and its 1.5 px sliver threshold).
  - **`batteryh` / `batteryv` are fully declarative zero-`Ink` cards**
    (`ui/cards/batteryh.ron`, `ui/cards/batteryv.ron`) — the decision taken
    this session was a new generic **`Battery`** scene item rather than a
    hybrid: the gauge art moved out of `cards/shared.rs` into `scene.rs`
    (`push_battery_icon_parts` + `draw_water`), so the scene draws the Rust
    drawer's own geometry (per-orientation auto box when `w`/`h` are `0`). With
    it: the centered percent (inside a full-card `Stack`, because a card-level
    `Text` has no implicit width), the pane-1 metric rows on an 18 px pitch
    right-anchored 38 px from the card's right edge, the power-save pill as a
    `Hit` with `bottom: true` + a negative width span, and the pane dots. The
    pane gates fold in the no-battery guard, so an absent battery leaves the
    header and the empty caption only, exactly like the drawer's early return.
- **Engine additions**: `Battery { value, max, level, vertical, visible }`,
  `Hit { bottom }` (the `Text.bottom` mirror) + negative `Hit` spans, and
  `Fader { head, centered, visible }`, `Column.valign`,
  `span_px` for fader/row/column/stack/toggle boxes, `Hit.visible`,
  `SceneItem::Tiles` + `SceneTile` + `SceneValues::tiles()`, `Row.min_card_h`,
  `Row.halign`, `Bar.vertical`, `Surface { stroke, stroke_color }`,
  `SceneHit: Debug`, `vars::card_scenes_dir()`, a global `clear` value.
- **Engine fixes**: a right-anchored `Text` inside a container is no longer
  widened to the fill (that pushed nested labels a whole box-width right);
  `Hit` regions now honour their `visible` gate; **`Rows.key_base` is now
  `Option<u32>`**, so a list that omits it registers NO row regions — bare
  `index` keys used to squat on the global 1 / 2 / 3 keys, and 16 committed
  scenes (battery, alarms, cpu, mem, docker, fans, notes, …) were quietly
  doing it. All 9 live scenes that DO want regions were moved to the explicit
  `Some(n)` form.
- **Config rot caught + guarded**: `activewin.ron`, `pkgupdates.ron`,
  `clipboard.ron` had `visible: "x"` (must be `Some("x")`) — they had been
  failing to parse and silently falling back to their Rust drawers.
  `every_live_card_scene_parses` now parses every file in `ui/cards/`.
- **Two unterminated `r#"` raw strings** in this file's test module had been
  eating ~240 lines of test code (10 phantom-missing tests, 1 bogus failure).
  Fixed; both are a reminder that a missing string terminator in Rust fails
  *silently* by swallowing the rest of the file.
- **New shell helpers for the battery pair**: `Shell::power_save_state()`
  (the cards' Rust drawers were the only `load_power_save` callers and they no
  longer run, so the pill and the legacy `power_save_on` binding read the state
  file directly) and `Shell::battery_dots_hover(rect)`; `draw_card_scene` now
  publishes `battery_h_rect` / `battery_v_rect` in its zero-`Ink` branch, so
  the swipe hit-test and the dots gate still see the card.
- **One deliberate difference**: the drawer's power-save pill grows its click
  region 3 px past the capsule on every side; the scene registers the capsule
  itself.
- **Next**: `eq` (parked behind the zeneq halt), then whatever is picked out of
  the remaining `Ink` cards. `pane_dots` / `card_pane_flip_n` / `battery_dots`
  now have zero `Ink` consumers, so they can move into `scene.rs` whenever the
  last drawer that calls them is converted. See the gotchas list in
  `NEXT_STEPS.md` (negative spans, `Some(...)` wrappers, `Bar.max`,
  `Rows.key_base`, card-level `Text` width, `bottom:` offsets).
...
- 🟢 **cpu + mem fully declarative — zero Ink LANDED (2026-09-24)**: both gauge
  cards drop their Rust bodies (Rust drawers now dead for these ids). Header
  (cpu keeps `{cpu_meta}` mono meta); a left-anchored bead-ring `Ring`
  (36 beads, dot 4, radius 26, `center_y` only — x-hugging the left like the
  Rust drawer: `cx` 38 = r + pad/2 + 5) with the hero `%` `Text` centered on
  the ring center (`center: true`, semibold 15); metric `Rows` (label fg2 · 8.5
  mono value right-anchored at the card pad via `edge: Some(14.0)`). The cpu
  gauge + hero ink follow the Rust warn/crit ladder through a per-frame
  dynamic color `value("cpu_gauge_col")` (fg → warn ≥ 70 → danger ≥ 90);
  cpu temp row is inked per-frame (warn > 65 / danger > 80) via `col_colors`.
  mem adds the bottom swap `Bar` (`value: "{swap_frac}"`, `max: 100`, gated by
  the `swap_show` toggle). New values: `cpu_ring`, `cpu_rows`, `cpu_gauge_col`
  (color), `mem_rows`, `swap_frac`, `swap_show`. 256/256 tests green (the lone
  pre-existing `rows_edge...` hedge-test still fails on an unrelated
  TrailingCharacters parse), bin clean.
- `procmon.ron`, `journaltail.ron` (DONE 09-17), `systemdunits.ron` (DONE 09-17), `smarthealth.ron` (DONE 09-17), `sensors.ron`, `therbl_zones`, `conninfo.ron` (DONE 09-17), `worldclock.ron`, `fans.ron`, `diskio.ron`, `network.ron`, `topproc.ron`
