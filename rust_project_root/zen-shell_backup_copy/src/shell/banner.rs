...
impl Shell {
    ...
    /// Every token on the parked "Banner chips" tray (edit mode): unused chips
    /// in canonical order, then all separator presets, then the smart filler.
    ...
    pub(crate) fn parked_banner_tokens(&self) -> Vec<BannerToken> {
        let mut v: Vec<BannerToken> = self
            .unused_banner_items()
            .iter()
