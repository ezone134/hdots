...
impl Shell {
    ...
    fn draw_audiorec_recorder(&mut self, v: &mut Vec<Cmd>, x: f32, y: f32, w: f32, h: f32, pal: &Pal) {
        ...
        card_header(v, pal, x, y, w, pad, "Recorder", Some(ICON_MIC), Some((&meta, true)), self.scale, self.card_show_title, self.card_show_glyph);

        // "›" chevron button top-right → recordings pane
        ...
        if recording {
            ...
            v.push(Cmd::Rect { x: x + pad + self.scale.s(2.0), y: by + (bh - d) / 2.0, w: d, h: d, r: d / 2.0, color: RED });
        }
        ...
        ui::text(v, x + pad + self.scale.s(16.0), by + self.scale.s(7.0), &lbl, self.scale.fs(9.5), if recording { mix(pal.fg, RED, 0.15) } else { pal.fg }, false);
        ...
        for i in 0..n {
            ...
            if hov || armed {
                v.push(Cmd::Rect {
                    ...
                    h: row_h,
                    r: self.scale.s(5.0),
                    color: if armed { mix(ui::raised_hl(&pal), RED, 0.28) } else { ui::raised_hl(&pal) },
                });
            }
            ...
            ui::caption(v, x + pad + self.scale.s(2.0), ry + self.scale.s(5.0), &shown, self.scale.fs(8.5), if armed { mix(pal.fg, RED, 0.2) } else { ui::fg2(&pal) }, true);
            ...
            if armed {
                ui::text(v, d_gx, ry + self.scale.s(5.0), ICON_CLOSE_FILL, self.scale.fs(9.0), mix(RED, pal.fg, 0.15), true);
            } else {
                ui::text(v, d_gx, ry + self.scale.s(5.0), ICON_RECYCLE, self.scale.fs(9.0), if d_hov { mix(RED, pal.fg, 0.25) } else { ui::fg3(&pal) }, true);
            }
            ...
            ui::text(v, p_gx, ry + self.scale.s(5.0), ICON_PLAY, self.scale.fs(9.0), if p_hov { pal.acc } else { ui::fg3(&pal) }, true);
