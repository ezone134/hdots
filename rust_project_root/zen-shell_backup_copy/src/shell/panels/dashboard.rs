...
impl Shell {
        ...
        let sx = lt.scroll_x;
        let sy = lt.scroll_y;
        let canvas_w = lt.canvas_w;
        let canvas_h = lt.canvas_h;
        ...
        if editing && self.dash_enabled {
            ...
            let gy1 = (((sy - lt.grid_top + _h) / py).ceil() as i64).max(0) as usize;
            ...
            for gy in gy0..=gy1.min(crow) {
                for gx in gx0..=gx1.min(ccol) {
                    ...
                    if ex >= 0.0 && ex <= w - 3.0 && ey >= lt.grid_top - 1.0 && ey <= _h - 3.0 {
        ...
        let cursor_px = if editing { self.cursor } else { None };
        ...
        for (i, l) in layouts.iter().enumerate() {
            ...
            if let Some((card, gx, gy)) = drag_state {
                if card == l.card {
                    if let Some((mx, my)) = cursor_px {
                        x = (mx - gx).clamp(lt.pad, (canvas_w - lt.pad - rw).max(lt.pad));
                        y = (my - gy).clamp(lt.grid_top, (canvas_h - lt.pad - rh).max(lt.grid_top));
                        fluid = true;
                    }
                }
            }
            ...
            v.push(Cmd::Rect { x, y, w: rw, h: rh, r: lt.card_r, color: card_bg });
                l.card.draw(self, v, x, y, rw, rh, pal);
                if editing {
        ...
        if editing {
            if let (Some((card, dx, dy)), Some((mx, my))) = (drag_state, cursor_px) {
                if self.edit_drag_from_tray {
                    ...
                    let fy = (my - dy).clamp(lt.grid_top, (_h - lt.pad - fh).max(lt.grid_top));
                    v.push(Cmd::Rect { x: fx, y: fy, w: fw, h: fh, r: lt.card_r, color: ui::hover(&pal) });
                    card.draw(self, v, fx, fy, fw, fh, pal);
                }
            }
        }
        }
        ...
        // this mask (layer 3) is drawn after cards.
        if editing {
            lt.draw_chrome_mask(v, w);
        } else if !self.banner_collapsed() {
            v.push(Cmd::RectConcave {
                x: 0.0,
                y: lt.strip_top,
                w,
        ...
        let over_y = (canvas_h - _h).max(0.0);
        let over_x = (canvas_w - w).max(0.0);
        ...
        if over_y > 1.0 {
            ...
            let ty = (lt.grid_top + self.scale.s(4.0)).min(_h - tw - self.scale.s(8.0));
            let th = (_h - ty - self.scale.s(6.0)).max(20.0);
            if visible {
                let thumb_h = (th * (_h / canvas_h).min(1.0)).max(20.0);
        ...
        if h_rail {
            ...
            let tyy = _h - thh - self.scale.s(4.0);
            ...
            if visible {
                ...
                if over_x > 1.0 {
                    let thumb_w = (tw * (w / canvas_w).min(1.0)).max(30.0);
    ...
    /// on grid + edit chrome.
    fn layout_banner(&mut self, v: &mut Vec<Cmd>, w: f32, lt: &Layout, cursor_px: Option<(f32, f32)>, pal: &Pal) {
        ...
        // the strip cells, so it closes by pressing anywhere else.
        if self.connectivity_open && !self.dash_edit {
