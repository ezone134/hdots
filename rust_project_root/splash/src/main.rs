//! splash — GPU-accelerated Wayland splash screen.
//!
//! Reads $states2/splash_vars (JSON), renders text on a solid background,
...
fn parse_vars() -> Vars {
    let states2 = env::var("states2").unwrap_or_else(|_| "/tmp/tw_hypr_conf/states2".into());
    let path = PathBuf::from(states2).join("splash_vars");
    let content = fs::read_to_string(&path).unwrap_or_default();
    serde_json::from_str(&content).unwrap_or_else(|e| {
        eprintln!("splash: parse splash_vars: {e}");
        // Return defaults
        Vars {
            w: 100.0,
            h: 100.0,
...
fn load_fonts(db: &mut Database, wanted: &[String]) {
    ...
    if loaded == 0 {
        debug_log("splash: no wanted fonts found in scan, loading system fonts");
        db.load_system_fonts();
    } else {
        debug_log(&format!("splash: loaded {loaded} wanted font files"));
        db.load_system_fonts();
    }
}
...
fn rasterize(
    ...
    debug_log(&format!("splash: rasterize text={text:?} family={family} size={size} w={w} h={h} non_zero_px={non_zero}"));
    Some((w, h, pixels))
}
...
impl GlState {
    ...
    fn draw(
        ...
        let gl = &self.ctx;
        let alpha_f = (alpha as f32 / 100.0).clamp(0.0, 1.0);
        unsafe {
            static mut DRAW_COUNT: u32 = 0;
            DRAW_COUNT += 1;
            let draw_n = DRAW_COUNT;
            if draw_n <= 3 {
                let mut msg = format!("splash: draw #{draw_n} w={w} h={h} bg=0x{bg:06x} rounding={rounding} alpha={alpha} textures={}\n", textures.len());
...
struct App {
    ...
    // (Esc / Enter close the splash like rofi) instead of being keyboard-dead
...
impl App {
    /// Unconditionally tear the splash down (Esc / Enter / keyboard-leave /
    ...
    fn init_textures(&mut self) {
        ...
        match unsafe { GlState::init(display_ptr, layer.wl_surface(), w, h) } {
            ...
            Err(e) => {
                eprintln!("splash: GL init: {e}");
                std::process::exit(1);
            }
        }

        let gl = self.gl.as_ref().unwrap();
        let fg = (self.vars.fg_u32() << 8) | 0xff; // 0xRRGGBB → 0xRRGGBBAA
    ...
    fn remove_capability(
        ...
        if capability == Capability::Keyboard {
            ...
            if !self.cap_exhausted {
                ...
                debug_log("splash: keyboard capability dropped — closing");
                self.close();
            }
        }
    }
...
impl KeyboardHandler for App {
    ...
    fn leave(
        ...
        // when it loses focus.
        debug_log("splash: keyboard leave — closing");
        self.close();
    }
    fn press_key(
        &mut self,
        ...
        match event.keysym {
            Keysym::Escape => {
                debug_log("splash: Esc pressed — closing");
                self.close();
            }
            Keysym::Return | Keysym::KP_Enter => {
                debug_log("splash: Enter pressed — closing");
                self.close();
            }
            _ => {}
        }
...
fn debug_log(msg: &str) {
    ...
    let _ = std::fs::OpenOptions::new().create(true).append(true)
        .open("/tmp/splash_debug.log")
        .and_then(|mut f| { writeln!(f, "{msg}") });
}

fn main() {
    let _ = std::fs::write("/tmp/splash_debug.log", "\n--- splash start ---\n");
    ...
    // Connect Wayland
    let conn = Connection::connect_to_env().expect("splash: wayland");
    let (globals, mut event_queue) =
        registry_queue_init::<App>(&conn).expect("splash: globals");
    let qh = event_queue.handle();

    // Bind protocols
    let compositor = CompositorState::bind(&globals, &qh).expect("splash: wl_compositor");
    let layer_shell = LayerShell::bind(&globals, &qh).expect("splash: wlr-layer-shell");
    ...
    // Grab the keyboard so Esc / Enter dismiss the splash like rofi. Exclusive:
    // no other client receives keys while we're up (a splash is transient).
    layer.set_keyboard_interactivity(KeyboardInteractivity::Exclusive);
    layer.commit();
    conn.flush().expect("splash: flush");
    ...
    event_queue
        .roundtrip(&mut app)
        .expect("splash: roundtrip");

    // Init GL + upload all textures
    app.init_textures();
    debug_log(&format!("splash: after init: configured={:?} gl={} b_tex={} s_tex={} anim={}",
