...
- **Global env vars are the source of truth — never hardcode paths.** The
  ...
  (`hl.env`): `hypr_conf`, `states`, `states2`, `hypr_global`, `hdots`,
  `hypr_cache`, `hypr_bin`, `hypr_sources`, … They are ALWAYS set in the live
  ...
  `${hypr_conf%/*}/hypr_global`). The `*_body` helper files live
...
- **tmpfs-first, sync-at-session-end (minimal SSD writes).** Everything runs
  ...
  `$states2`, `$hypr_cache` and the live quickshell config are ALL under
  ...
  (rsync `$states/` → `$hdots/states/`, `$hypr_conf/quickshell/` →
...
Two RAM-backed dirs under `$hypr_conf` (`states` and `states2`) hold every
...
| --- | --- | --- | --- |
| `$hypr_bin` = `/tmp/<user>_hypr_conf/hypr_bin` | ✅ RAM | ✅ wiped, recopied from `$hdots/hypr_bin/` | ❌ (persistent copy is canonical) |
| `$hypr_sources` = `/tmp/<user>_hypr_conf/sources` | ✅ RAM | ✅ wiped, recopied from `$hdots/sources/` | ❌ |
| `$states` = `/tmp/<user>_hypr_conf/states` | ✅ RAM | — | ✅ `sync_back` → `$hdots/states/` |
| `$states2` = `/tmp/<user>_hypr_conf/states2` | ✅ RAM | — | ❌ never (ephemeral) |
| `$hypr_cache` = `/tmp/<user>_hypr_conf/hypr_cache` | ✅ RAM | — | ⚠️ accent files only |
| live quickshell = `~/.config/quickshell` → `/tmp/<user>_hypr_conf/quickshell` | ✅ RAM | ✅ recopied from `$hdots/.config/quickshell/` | ✅ `sync_back` → `$hdots/.config/quickshell/` |
...
2. **`require('env')`** → `~/.config/hypr/env.lua` exports every global via
   `hl.env(...)`: `hypr_conf`, `states`, `states2`, `hypr_bin`,
   `hypr_sources`, `hypr_global`, `hypr_cache`, `hdots`, PATH — all under
...
3. **`hl.on("hyprland.start")` #1** → `exec_cmd("$hdots/launch_bins/prelaunch_hypr")`
   ...
   - `main_logic` **WIPES the tmpfs runtime**: `rm -rf $hypr_bin/*
     ...
     `$hypr_conf/quickshell`, `$hdots/misc/*` → `/tmp/hypr_misc/misc/`, then
     `$hypr_sources/scripts/*` → `$hypr_conf/scripts/`.
...
- **quickshell** is the deliberate exception: it runs live from tmpfs
  (`~/.config/quickshell` → `/tmp/tw_hypr_conf/quickshell`) so reloads are
...
- Never spawn `dirname`/`basename`/`readlink`/`pwd` to derive a script's own directory —
  ...
  `$hypr_conf`, `$qs_scripts`) are already exported, so a `$PATH`-resolved CLI never
...
- `/tmp/tw_hypr_conf/states2/s` -> channel letter (currently `d`),
  `/tmp/tw_hypr_conf/states/current_wall_<ch>` -> image path. Fallback: parse
...
on creation).- The `*_main` CLIs fall back to the same dotfiles paths as
  `SystemSettingsManager` (`$hypr_conf`/`$states`/…), so they work from any
