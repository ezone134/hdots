...
WlSessionLock {
    ...
    readonly property string resolveScript: [
        'ch="$(cat /tmp/tw_hypr_conf/states2/s 2>/dev/null)"',
        'wall=""',
        'if [ -n "$ch" ] && [ -r "/tmp/tw_hypr_conf/states/current_wall_$ch" ]; then',
        '    IFS= read -r wall < "/tmp/tw_hypr_conf/states/current_wall_$ch" 2>/dev/null',
        'fi',
        'if [ -z "$wall" ]; then',
