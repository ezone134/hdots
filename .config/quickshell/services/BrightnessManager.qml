...
Scope {
    ...
    // (self-guarded: no-op unless $states2/gamma > 0), so the slider never
    // forks shader_main on every tick for nothing.
    Process {
        id: gammaResetProc
        command: ["bash", "-c", "g=0; [[ -r \"$states2/gamma\" ]] && read -r g < \"$states2/gamma\"; if (( g > 0 )); then printf '0\\n' > \"$states2/gamma\"; shader_main reset 2>/dev/null || true; fi"]
    }
